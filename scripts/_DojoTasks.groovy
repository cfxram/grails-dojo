...
target(createOptimizedDojoBuild: "This is the main target for this script."){
  print "Creating an Optimized Dojo build..."
...
target(downloadDojoSource: "This will download the source version of Dojo.") {
  print "Downloading Dojo ${Dojo.version} source files..."
  ...
  //event("StatusUpdate", ["Done Downloading Dojo ${Dojo.version} source files."])
}
...
target(createDojoCssBuild: "This will create a css build file.") {
  print "Creating Custom Dojo CSS build file..."
  ...
  //event("StatusUpdate", ["Done Creating Custom Dojo CSS build file."])
}
...
target(createDojoProfile: "This will create the dojo profile js file") {
  print "Creating dojo profile js file..."
  ...
  //event("StatusUpdate", ["Done Creating dojo profile js file."])
}
...
target(buildDojo: "This will run shrinksafe to create an optimized version of dojo") {
  print "Runnning shrinksafe to create an optimized dojo..."
...
target(copyDojoToStage: "This will copy the optimized dojo release to stage.") {
  print "Copying optimized dojo release to the staging area..."
...
target(copyDojoToApp: "Copies the optimized dojo release to application.") {
  print "Copying custom dojo build to the application..."
...
target(cleanup: "This will copy the optimized dojo release to application.") {
  print "Cleaning up custom dojo build tmp files..."
  //event("StatusUpdate", ["Cleaning up custom dojo build tmp files."])

...
class DialogTagLib
{
  ...
  def dialogResources = {attrs, body ->
    out << dojok.require(modules: ['dijit.Dialog'])
  }
  /**
  ...
  def closeDialogButton = {attrs, body ->
    ...
    def id = attrs.remove("id") ?: "dojo_ui_dialog${Util.randomId()}"
    out << """ <a target="_self" ${Util.htmlProperties(attrs)} id="${id}" onclick="dijit.byId('${dialogId}').hide(); ${onclick}">${label}</a> """
  }
  /**
  ...
  def openDialog = {attrs, body ->
    ...
    out << """ <a href="#" ${Util.htmlProperties(attrs)} onclick="dijit.byId('${dialogId}').show()">${body()}</a> """
  }
}